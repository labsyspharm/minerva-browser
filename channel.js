...
const render_tile = (props, uniforms, via) => {
  ...
  console.log(`Rendered ${data.channels.length} channels to gl:`);
...
const customizeTileSource = (HS, tileSource) => {
  ...
  return {
    ...
    getTileCacheDataAsContext2D: function(cache) {
        ...
        const h = out.height;
        const w = out.width;
        console.log(`Copying ${w}x${h} px from gl:`);
        console.log(out, ctx);
...
const linkShaders = (props) => {
  props.viewer.addHandler('tile-drawing', (e) => {
    //console.log(e); //TODO
  });
